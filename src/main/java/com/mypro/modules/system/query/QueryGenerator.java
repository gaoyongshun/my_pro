package com.mypro.modules.system.query;

import com.alibaba.fastjson.JSON;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.mypro.common.constant.CommonConstant;
import com.mypro.common.util.ConvertUtil;
...
//			if(ConvertUtil.isNotEmpty(c) && c.startsWith(SQL_RULES_COLUMN)){
...
//				}
				...
				if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
					...
					final String field = ConvertUtil.camelToUnderline(name);
					if(vals.length>1) {
						queryWrapper.and(j -> {
	...
	public static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap) {
		...
		if (ConvertUtil.isNotEmpty(column) && ConvertUtil.isNotEmpty(order)) {
			//字典字段，去掉字典翻译文本后缀
			...
			if (order.toUpperCase().indexOf(ORDER_TYPE_ASC)>=0) {
				queryWrapper.orderByAsc(ConvertUtil.camelToUnderline(column));
			} else {
				queryWrapper.orderByDesc(ConvertUtil.camelToUnderline(column));
			}
		}
	}
	...
	public static void doSuperQuery(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap) {
		if(parameterMap!=null&& parameterMap.containsKey(SUPER_QUERY_PARAMS)){
			...
			for (QueryCondition rule : conditions) {
				if(ConvertUtil.isNotEmpty(rule.getField()) && ConvertUtil.isNotEmpty(rule.getRule()) && ConvertUtil.isNotEmpty(rule.getVal())){
	...
	private static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
		...
		name = ConvertUtil.camelToUnderline(name);
	...
	public static String getSingleQueryConditionSql(String field,String alias,Object value,boolean isString) {
		...
		field =  alias+ConvertUtil.camelToUnderline(field);
